 ...
 *  the module is a passive one-way analyzer, never creates fireflies)? */
enum class JarRole { CREATION, WATCHING }

/**
 * The three research modules nightjar demos. Order is the home
 * screen's row order: Module 3 (the primary phone-to-phone demo) first, then
 * Module 1, then Module 5. Real content lands in later tasks — this task only wires
 * navigation to a stub per module.
 *
 * Added [description] — a one-line, first-run answer to "what does this
 ...
 *
 * Added [jarName]/[jarRole] — the disguise-themed name and
 * shelf role shown on the Firefly Jar surface. Not used
 * by [ModulePicker] itself; consumed by the jar shelf/detail screens.
 *
 * Added [jarChannel] — the one-word carrier a firefly
 * travelled over, for the firefly-detail popup's metadata row (DESIGN_SPEC.md §5 1c).
 * It lives here for the same reason [jarName] does: the app allows exactly
 * two exhaustive per-module branches, and the detail screen is not one of
...
/**
 * Home screen. Dense, scannable list — no cards, no icons, no dividers (matches a sibling
 * app's session-picker row precedent). Exactly 4 rows; nothing to sort by
 * frequency yet since the list is fixed at this scope.
 *
 * U-01: this is the entry point of the long-press reveal from the jar
...
fun ModulePicker(
    ...
    onBack: () -> Unit,
    // "start the trail again" footer link -- default no-op keeps this composable usable without
    // a trail store in scope (there is no @Preview for this screen today, but this matches every
    // other optional-hook default in this app's jar-mode screens).
    onRestartTrail: () -> Unit = {},
) {
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        // Plain footer link, same labelLarge/TextSecondary styling the "back to the jar"
        // link above already uses.
        Box(
            modifier = Modifier
