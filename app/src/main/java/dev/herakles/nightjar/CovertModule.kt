 ...
 *
 *  - Module 3 acoustic modem -> `CovertCarrier<PcmAudio>`
 *  - Module 5 acoustic detector -> `CovertDetector<PcmAudio>`
 *  - Module 1 image LSB codec -> `CovertCarrier<android.graphics.Bitmap>`
 *  - Module 1 image steganalysis -> `CovertDetector<android.graphics.Bitmap>`
 *
 * Design split (two interfaces, one family):
 *  - A [CovertCarrier] is a *reversible codec*: `payload -> carrier -> payload`. It is the
 *    natural home for the round-trip lossless guarantee.
 *  - A [CovertDetector] is a *one-way analyzer*: `sample -> score`. It never recovers a
 *    payload (that is exactly the passive/defensive posture of Module 5).
 ...
 * transport. `AudioTrack`/`AudioRecord` playback and capture (the streaming I/O, elsewhere
 * called the "streaming audio-capture/decode architecture") live in the UI/transport layer
 * that drives these interfaces. Keeping the contract at `ByteArray <-> PcmAudio` / `Bitmap`
 * is what makes the loopback self-test a pure unit test: feed an
 ...
 */
enum class ModuleId {
    /** Module 1 — image LSB steganography encode/decode. */
    IMAGE_LSB_CODEC,
    /** Module 1 — image chi-square/RS steganalysis. */
    IMAGE_STEGANALYSIS,
    /** Module 3 — acoustic FSK modem encode/decode. */
    ACOUSTIC_MODEM,
    ...
    AUDIO_STEGO_CODEC,
    /** Module 2 — blind audio steganalysis (v5 addition, `AudioStegDetector`). */
    AUDIO_STEGANALYSIS,
    /** Module 1 — sturdy image firefly codec (v6 addition, `SturdyImageCarrier`):
     *  dither-QIM on a logical luminance grid, survives JPEG recompression/resize where the
     *  exact-LSB codec cannot — the "Sturdy image technique (v6)." Its own technique,
     *  sibling to [IMAGE_LSB_CODEC], not a replacement. */
    STURDY_IMAGE_CODEC,
}
...
/**
 * A reversible steganographic codec: hides [ByteArray] payloads inside a carrier of type [C] and
 ...
 *  - [encode] accepts only operator-supplied benign payloads; the contract cannot
 ...
 *    whose `payload` equals the input.
...
interface CovertCarrier<C> : CovertModule {
     ...
     * Acoustic modem: 1024. Image codec: capacity-derived from the bitmap.
     */
...
sealed interface DecodeResult {
     ...
     * @param payload the recovered bytes (equals the original within the envelope).
     * @param correctedByteErrors FEC-corrected byte count (acoustic RS);
...
enum class DecodeFailure {
    ...
    NO_PAYLOAD_FOUND,
    /** Header failed magic/version/header-CRC validation (acoustic: header_crc). */
    HEADER_INVALID,
    ...
    UNRECOVERABLE_FEC,
    /** Payload recovered but its integrity checksum did not match (acoustic: CRC-32 mismatch). */
    INTEGRITY_MISMATCH,
}
 ...
 * self-detecting the app's own Module 3 transmission) and the image steganalysis
 ...
 * that state is an implementation detail held inside the instance. Each
...
interface CovertDetector<S> : CovertModule {
     ...
     * tunable false-positive/false-negative knob (acoustic threshold;
