...
/**
 * Android adapter for [SturdyImageCarrier]. The carrier itself is pure Kotlin (no
 * `android.*`), so this small file supplies everything the
 ...
 * the ~640px robustness floor), and the JPEG export step (the survival matrix is
...
class BitmapPixelSurface(private val bitmap: Bitmap) : SturdyImageCarrier.PixelSurface {
     ...
     * [Bitmap.getPixels] call per ROW instead of one [Bitmap.getPixel] JNI call per PIXEL: up to
     * [Bitmap.width] x [Bitmap.height] calls otherwise, tens
...
enum class SturdyCoverRejection {
    /** Long side under [STURDY_MIN_COVER_LONG_SIDE_PX] -- known risk: "Small
...
/**
 * Send-side cover prep: downscales [bitmap] so its long side is at most
...
/** Known risk: survival falls below ~512-640 px long side. */
...
/**
 * JPEG quality for the sturdy send path. The survival matrix's q50-95 range is
 ...
 * harness's file-size pass (24 real Kodak photos at their native <=768px long
