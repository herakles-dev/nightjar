...
/**
 * Module 1 — image LSB steganography codec.
 *
 ...
 * the LSB steganography literature. This is the *raw-pixel* LSB technique from
 * that literature, not the DCT-coefficient ("Javid steganography") variant that survives JPEG
 * re-compression — Module 1's scope here is raw-pixel LSB encode/decode plus its steganalysis
 * counterpart, so this codec only guarantees a lossless round trip through lossless
 * bitmap handling ("In scope").
 *
 * ## Framing (analogous to the acoustic protocol's framing)
 *
 ...
 * ("Image codec: capacity-derived from the bitmap").
 *
...
class ImageStegoCarrier(private val coverImage: Bitmap) : CovertCarrier<Bitmap> {
     ...
     * covering row range, not one [Bitmap.getPixel] JNI call per bit. [decode] calls this at most three times (header, payload, trailer) per
    ...
    companion object {
        ...
        /** Header magic byte 'N' — matches the acoustic header's magic. */
