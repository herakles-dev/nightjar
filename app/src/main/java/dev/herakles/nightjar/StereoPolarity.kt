...
/**
 * v5 addition. Pure time-domain L/R polarity statistics for a
 ...
 * constructs an [AudioStegoCarrier] or calls `decode()`. `AudioStegDetector`
 ...
 * disagree — computing the same statistic twice, once loosely for a
 * detector's threshold and once precisely for a picture, is exactly the kind of drift
 * known as "the #1 integration hazard."
 *
 ...
 *   — measured −0.99936 to −0.99998 on real stego output); ordinary stereo
 ...
 *   residual strip this view draws.
 ...
 *   energy) — individual polarity cycles are only visible at this zoomed-in scale ("at
 *   whole-clip scale, a waveform envelope is symmetric, so polarity is invisible").
 ...
 * whatever stereo bytes a device WAV import or a backup-restore hands it (the
 ...
 * hops ("the loudest 20 ms window (by L energy, 10 ms hop)") — never
