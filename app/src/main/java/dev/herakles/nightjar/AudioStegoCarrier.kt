...
/**
 * Module 2 — audio steganography codec.
 *
 ...
 * [AudioStegoTechnique] values are implemented (note on capacity: "this spec
 ...
 * the LSB steganography literature: "convert a stereo file to dual-mono (left and
 ...
 * a usable ~12.5 bytes/sec of capacity for a demo-length clip.
 ...
 * default strength is a matter of human verification ("verified by ear on real hardware, not asserted"),
 ...
 * The LSB steganography literature's account of "spectrogram LSB embedding" is
 ...
 *
 * **Backward compatibility:** v1 carriers — every spectrogram-LSB firefly caught before
 ...
 *
 * **Measured lossy-codec survival — narrower than "robust" implies:** MFSK
 ...
 * live modem does.
 *
 * Clipping fix: [TONE_AMPLITUDE] × up to [MFSK_TONE_COUNT] simultaneous
 ...
 *
 * **Click fix**: the gain fix above stops [MFSK]'s samples from clipping, but
 ...
 * dual-mono trick, a different, simpler technique), so this section cites the source directly,
 * matching the precedent the steganography literature already sets for citing an external paper
 ...
 *
 * **Fidelity: owner-verified on real headphones (2026-09-22), and it confirms the metrics
 * above rather than contradicting them**: `AudioSampleCover.SPOKEN_WORD` (payload "nightjar test
 * payload") played back clean -- cover and working clip indistinguishable by ear, matching its
 * smaller measured residual. `AudioSampleCover.SOFT_SYNTH` (same payload) produced an audible
 * gurgle/warble on the working clip. This is the same bin-collision-avoidance story
 * [PHASE_BASE_BIN]'s own KDoc already tells, one level down: bin 20 avoids SOFT_SYNTH's
 * *fundamental and harmonics*, but the forced +-PI/2 phase reset at the start of every group is
 * still a discontinuity in a *coherent tone*, and a listener hears that discontinuity as
 * warble even when it lands well clear of any one strong partial. SPOKEN_WORD has no coherent
 * tone there to clash with -- its energy is spread across the whole passband -- so the same
 * discontinuity is masked. Net: this technique is honestly usable on broadband/noise-like
 * covers, not on tonal/harmonic ones, and that is a property of literature phase coding itself
 * (Bender's own validation was on speech), not a bug in this implementation. Round-trip
 * correctness was never in question, only fidelity, and only on tonal covers.
 *
 ...
 * ## Framing
 *
 * Identical shape to [ImageStegoCarrier]'s frame (acoustic framing adapted
...
class AudioStegoCarrier(
    ...
    private fun decodeSpectrogramLsb(carrier: PcmAudio): DecodeResult {
        ...
        if (headerVersion == VERSION) {
            // v1: dense, byte-for-byte the original algorithm -- unchanged.
    ...
    companion object {
        ...
        /** Header magic byte 'N' — matches every other module's framing. */
         ...
         * content, which is where the LSB steganography literature notes spectral
         ...
         * tones at this amplitude can sum past int16 range on their own (measured 96-141
         * saturated samples per stego pre-fix; byteValue == 0xFF's actual
...
/**
 * The three audio steganography techniques Module 2 supports, one
...
enum class AudioStegoTechnique {
     ...
     * the LSB steganography literature calls "the simple trick," not the DFT
