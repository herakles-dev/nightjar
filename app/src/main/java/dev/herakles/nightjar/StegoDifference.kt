...
/**
 * v5 addition (design-v5.md §3, gate-24). Cover-vs-stego difference computation for
 ...
 *
 * Nightjar never stores the cover a firefly was caught from — only the stego clip (gate-13,
 * spec.md INV-6). Both of this app's bundled covers ([dev.herakles.nightjar.modules.audiostego
 ...
 *
 * INV-8 (spec.md v5 addition): this file only ever draws conclusions it can verify from the data
 ...
 *  KDoc and design-v5.md §3.1's rejected-alternatives table for why re-derivation (not a stored
...
/**
 * Finds the unique bundled cover [stegoMono] was embedded from, or `null` if none fits — INV-8:
 * this never guesses. [candidates] is a `(label, supplier)` list so callers (`JarDetailScreen`'s
 ...
 * cover and its spectrogram-LSB stego, per design-v5.md §3.2 (measured against real
 ...
 *    itself — new, not shifted, spectral content. This is design-v5.md §3.3's finding: silent
 ...
 * front (design-v5.md §3's own worked examples: 6 to 106 frames depending on payload size).
 *
 ...
 *   design-v5.md §3.2's own display range).
 ...
 *   `>= 0.03` nats, or `null` if no bin qualifies. design-v5.md §3.2: the thresholded *set* of
 ...
 *  anywhere close to 30. Matches design-v5.md §3.2's stated rule and
 ...
 *  to decide a bin belongs to the embedded band -- design-v5.md §3.2: measured out-of-band (pure
