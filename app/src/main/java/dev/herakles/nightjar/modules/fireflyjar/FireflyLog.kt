 ...
 * Firefly Jar UI. `moduleId` stores [dev.herakles.nightjar.ModuleId.name] so the log
 * stays decoupled from that enum's Kotlin type.
 *
 * v2 adds media attachment fields: [carrierKind] ("IMAGE"|"AUDIO"|null), [mediaPath]
...
interface FireflyDao {
    /** Returns the inserted row's autogenerated id (Room's `@Insert` returning-`Long` form --
     *  no schema change; Room stays v2). A practice catch needs this id back
     *  immediately to mark it via [dev.herakles.nightjar.trail.TrailStateStore.markPractice]. */
    @Insert
    suspend fun insert(record: FireflyRecord): Long
    ...
    /**
     * Physical on-disk media usage. Two corrections over a naive
     ...
     *
     * Backs [FireflyRepository]'s reference-counted delete: once content-addressed filenames
     * land, two rows can share one on-disk file, so deleting that
...
/**
 * v1 -> v2: adds the media attachment columns. SQLite cannot add a NOT NULL column
