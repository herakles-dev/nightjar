...
/**
 * Makes a firefly's row and its attached media file inseparable. Every deletion
 ...
 *
 * Stage A rewired the shelf's clear-history path. Stage B added the shared
 ...
 *
 * Stage C made file deletion reference-counted via [deleteMediaFileIfUnreferenced].
...
class FireflyRepository(
     ...
     *
     * Truncated-write decision: chose (b) -- document that the orphan
     ...
     *
     * Returns the inserted row's id (a practice catch needs it immediately to mark
    ...
    /** Reclaims media files on disk with no row referencing them (crash-recovery sweep). */
