 ...
 *  has no dividers (matches the platform's other session-picker precedent:
...
/**
 * A cyan pulled from the same GitHub-dark color family the
 ...
 *      covert signal has actually been caught. This is the app's actual thesis —
 *      self-detectability is the defensive proof-of-concept nightjar exists
 ...
 * framing — recovered text appearing in [TextPrimary] already IS the feedback (a recording
 * accent should land only on the mic ring, never on the result transcript).
 */
 ...
 * `ui/theme/WorkshopButton.kt`). GitHub
...
// A warm dusk-meadow palette deliberately distinct from the cold scale above, scoped to
// Screen.JarShelf/Screen.JarDetail only. The technical screens above this comment
// keep using the palette already defined above, unchanged. Added for FireflyGlyphs.kt,
// which is the first consumer; JarShelfScreen.kt reuses the same constants
 ...
 *  purpose — the technical app's "the covert
