...
/**
 * Everything [TrailStateStore] persists ("progress survives process death").
 *
 * @param currentStep The step currently glowing ("one next step glows at a time"). Null
 ...
 *   [TrailStateStore.skip] ("the trail state simply stops
 ...
 *   [TrailStateStore.restart]'s KDoc). This is the only place practice status lives: never
 *   a Room column, so `FireflyRecord`/the v2 schema need no change to carry it.
 * @param welcomeSeen True once the
 ...
 * @param finaleDismissed True once the shelf's finale panel (all six dots lit, the
 ...
 * @param lastCompletedStep The step [TrailStateStore.advance] most recently completed,
