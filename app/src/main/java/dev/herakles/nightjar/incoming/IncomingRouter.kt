...
/**
 * Auto-detect-and-route pipeline core: given a file's
 * already-sniffed [SniffedType] and (for images) an already-decoded [Bitmap], tries every
 * technique nightjar knows and resolves to exactly one [IncomingOutcome].
 *
 ...
 * marker match, a bad header-CRC, or a nonsensical declared length) -- the "never guesses" rule
...
object IncomingRouter {
     ...
     * [padToFrameBoundary]), not [pcm] directly:
     ...
     *
     * [checkCancelled] threads cooperative cancellation into the acoustic-modem search --
     * [AcousticCarrier.decode]'s own `checkCancelled` overload,
     ...
     *  Catches `Throwable`, not just `Exception`: an
