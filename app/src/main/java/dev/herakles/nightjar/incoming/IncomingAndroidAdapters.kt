...
object IncomingAndroidAdapters {
     ...
     * nullable result so [IncomingPipeline] can report an
    ...
    /** Build notes: "reject > 50 MB." */
    const val MAX_INCOMING_FILE_BYTES = 50 * 1024 * 1024

    /** Build notes: "refuse exact-LSB attempts above ~24 MP (avoid
     ...
     * distinct [Bounded.TooLarge] rather than the same
    ...
    fun readBoundedBytes(
        ...
        // (defense in depth): the 50 MB bound already makes
     ...
     * without ever allocating pixel memory for it (as the distinct [Bounded.TooLarge]),
     * then decodes for real. [Bounded.Failed] for anything that
     ...
     * re-opening the source `Uri` a second time: a hostile
