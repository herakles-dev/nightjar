...
/**
 * v6 send plumbing (spec.md's v6 addition, amended INV-5, gate-33): jar-mode "send this firefly"
 ...
 * storage -- that split is the whole point of the amendment: jar-mode send is a private
 ...
 * is untouched by this file. INV-10 holds too -- no permission is declared or needed here, since
 ...
 *
 * This task (v6/W1-3, "send plumbing") only builds this utility -- no screen calls
...
object FireflyShare {
    ...
    /** Default staleness window for [sweepOutgoing] -- one hour, per spec.md's v6 addition. */
     ...
     * amended INV-5. Reports the send to [DebugProbe] (spec.md's v6 Runtime Verification Surface
     * addition: "the last send (technique, cache file bytes, authority used), cache-dir file
     * count") -- a no-op in release builds, same as every other `DebugProbe.report*` call site.
     *
     ...
     * dispatches this onto `Dispatchers.IO` there since it touches the filesystem (spec.md's v6
     * addition: "a one-line call; don't restructure MainActivity"). Resolves the real outgoing
 ...
 *
 * [IMAGE_STURDY] gets a `.jpg` extension (not `.png`) per spec.md's v6 addition naming
 * convention -- deliberately matching what a messaging app's own photo-send path would produce
 * anyway, since the sturdy technique (`architecture.md` § "Sturdy image technique (v6)") exists
 * specifically to survive that recompression. [IMAGE_EXACT] stays `.png`: the exact-LSB frame
 * only survives byte-for-byte as a lossless file (spec.md's "send it as a file" advisory).
 */
...
/** `yyyyMMdd_HHmmss` -- spec.md's v6 addition naming pattern (`IMG_yyyyMMdd_HHmmss.png`, etc).
