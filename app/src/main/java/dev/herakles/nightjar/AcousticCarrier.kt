...
/**
 * Module 3 acoustic FSK modem — Tx path.
 *
 * Implements the encoder half of `CovertCarrier<PcmAudio>` exactly per the "Acoustic
 * Protocol" spec (§1-§6). Every DSP constant (sample rate, frame/FFT size, tone
 ...
 *
 * `decode()` reverses that exact assembly: locate the START/END markers, FFT-demodulate
 ...
 *   the default phone-to-phone demo channel.
 * @param symbolRate which symbol-duration setting to use. Defaults to
 *   [NightjarAcoustics.SymbolRate.NORMAL].
 */
class AcousticCarrier(
     ...
     *
     * Wire format produced, in emission order:
    ...
    /**
     * Recover a payload from [carrier] by reversing [encode] step by step:
     *  1. **Locate the START marker**, in two stages:
     *     1a. *Coarse, frame-quantized candidate* — bounded sliding-window search.
     ...
     *     1b. *Sample-accurate refinement* — [refineMarkerOnsetSample] takes that
     ...
     *     buffer happens to be left (see step 3 for why that distinction matters).
     ...
     *     deliberately NOT derived from `carrier.size - startSample`: just as a real
     ...
     *  5. **RS-decode the payload+trailer blocks** (32-data/16-parity blocking,
    ...
    fun decode(carrier: PcmAudio, checkCancelled: () -> Unit): DecodeResult {
        ...
        // search. See this method's KDoc step 1a for why: a real capture has arbitrary
        ...
        // 1b. Refine to the TRUE, sample-accurate marker onset. See decode()'s KDoc
        ...
        // NOT derived from carrier.size, so it works identically whether or not the
        ...
        // stream occupies by mirroring encode()'s own RS(48,32) blocking --
        // deliberately NOT `carrier.size - dataStartSample`, since a real capture can have
    ...
    // ---------------------------------------------------------------------
    // Payload + trailer framing + RS(48,32) blocking
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // Tone -> nibble -> byte demodulation (reverses symbolFrequencies)
    // ---------------------------------------------------------------------
     ...
     * frame-quantized search; [refineMarkerOnsetSample] calls the
     ...
     * (bin-exact tones -> zero FFT leakage), so this comfortably separates
     ...
     * [refineMarkerOnsetSample] can compare magnitudes across candidate offsets instead
     ...
     * onset (see decode()'s KDoc step 1b for the field-test symptom this fixes).
     *
    ...
    /**
     * Ceiling integer division. Used by [decode] to independently recompute the coded
    ...
    // ---------------------------------------------------------------------
    // FFT — magnitude spectrum of one analysis frame
    // ---------------------------------------------------------------------
     ...
     * this protocol relies on. Adding a taper here would only spread each peak across
     ...
     * at a frame boundary — see this protocol's "one tone = one bin" alignment property).
     */
    ...
    companion object {
        ...
        /** Floor under log10 to avoid -Infinity dB for a silent/near-silent bin (demod). */
         ...
         * [refineMarkerOnsetSample]. The coarse candidate is provably within one
         ...
         * `data[offset, offset+length)`. Protects the header.
         */
 ...
 * [NightjarAcoustics.RS_PRIMITIVE_POLY]. `internal` (not `private`) so
 * [AcousticCarrier]'s decoder and this module's tests can reuse the same GF(2^8) arithmetic
 * instead of re-deriving it — the exact drift hazard shared interfaces are meant to prevent,
 * generalized from DSP constants to the FEC math built on top of them.
 *
 * [decode] uses roots `alpha^0 .. alpha^(nsym-1)` (fcr=0, matching [generatorPoly]):
 ...
 * (RS(14,6)) and the fixed/shortened 32-or-fewer-byte payload blocks (RS(48,32)) this protocol
 * calls for.
 */
internal object ReedSolomon {
    ...
    // ---------------------------------------------------------------------
    // Decode: syndrome check -> Berlekamp-Massey -> Chien search -> Forney
    // ---------------------------------------------------------------------
...
internal object GF256 {
    ...
    /** Multiplicative inverse in GF(2^8) (RS decode: Forney algorithm, Berlekamp-Massey). */
    fun inv(a: Int): Int {
