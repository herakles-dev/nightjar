...
/**
 * v5 addition. Plain JUnit4, no Robolectric — [matchCover] and
 ...
 * below come from `scratchpad/proto/run1.txt`/`run3.txt`/`run5.txt` (calibration runs against
 * these exact sources), and this file's tests independently re-measure
...
class StegoDifferenceTest {
     ...
     *  candidates as lazy `(label, supplier)` pairs over both bundled covers. */
    ...
    fun `matchCover identifies the right bundled cover for real SLSB stego output from each cover`() {
        for (cover in AudioSampleCover.entries) {
            ...
            // Measured right-cover ratios are 6.5e-5 / 1.8e-7 -- 1e-3 is a
            // comfortable margin under the 1e-2 accept threshold.
            assertTrue(
    ...
    // ---------------------------------------------------------------------
    // matchCover: withholds rather than approximates -- foreign clip, size mismatch,
    ...
    fun `matchCover still matches with plus-or-minus 1 LSB jitter on 1 percent of samples`() {
        ...
        assertNotNull("±1 LSB jitter on 1% of samples must not break the match", match)
    ...
    fun `matchCover rejects a simulated Random-drifted resynthesis of the same cover shape`() {
        ...
        // same sample sequence -- the stated failure mode this threshold must
        // catch.
        val rng = Random(999)
        ...
        for (cover in AudioSampleCover.entries) {
            ...
            for (strength in 1..4) {
                ...
                for (len in lengths.filter { it <= carrier.maxPayloadBytes }) {
                    ...
                    // The exact-span claim is calibrated (scratchpad/proto/run5.txt,
                    ...
                    // calibration run, and measuring them here (not just following the broader
    ...
    /**
     * Originally found: SPOKEN_WORD has genuinely all-zero "gap" frames between
    ...
    // ---------------------------------------------------------------------
    // Re-derivation determinism (kotlin.random.Random and
