 ...
 *
 * The malformed-input cases below assert "does not throw" rather than "returns null": Robolectric
 * 4.13's `BitmapFactory` shadow doesn't faithfully reproduce real Android's null-for-unparseable-
 * bytes contract for every shape of bad input (confirmed empirically -- it can synthesize a
 * placeholder `Bitmap` for a few garbage bytes, or throw for a truncated PNG header, where real
 * Android returns `null`/`outWidth == -1` either way). What this app's own code actually controls
 * (and [decodeBoundedBitmap]'s try/catch exists specifically for) is never propagating that as a
 * crash -- these tests hold the line on that contract without depending on the shadow's fidelity.
 */
