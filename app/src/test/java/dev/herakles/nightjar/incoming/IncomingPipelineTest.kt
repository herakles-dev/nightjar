...
/**
 * [IncomingPipeline.route] coverage: the router
 ...
 * that wraps them, and the one place several edge cases all actually land (the [Throwable]
...
class IncomingPipelineTest {
    ...
    // --- TooLarge is distinct from a generic read failure ---
    ...
    // --- the Throwable boundary ---

    @Test
    fun `a ContentProvider that throws IllegalStateException resolves to Unsupported, not a crash`() = runBlocking {
        // IllegalStateException specifically: previously, readBoundedBytes only caught IOException
    ...
    // --- baseline coverage through the pipeline, not just the router ---
