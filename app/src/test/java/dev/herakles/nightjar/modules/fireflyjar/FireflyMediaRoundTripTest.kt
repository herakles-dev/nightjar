...
/**
 * Verification for gate-17 (spec.md "Gates — v4 addition"): "carrier media is captured and
 * provably intact ... a round-trip test shows the bytes the media store wrote decode back to a
 * bitmap/PCM that still yields the original payload through that carrier's own `decode`." Task
 * #20, Stage B.
 *
 * The three creating jars (image LSB, audio-stego, acoustic modem) each now write their carrier
 * media through [FireflyMediaStore] at catch time (task #17/#18/#19 — see
...
class FireflyMediaRoundTripTest {
    ...
    fun imageCarrierPayloadSurvivesTheFireflyMediaStoreRoundTrip() {
        ...
        val payload = "firefly gate-17 image round trip - the ravens have landed."
    ...
    fun audioStegoMonoCarrierPayloadSurvivesTheFireflyMediaStoreRoundTrip() {
        ...
        val payload = "firefly gate-17 audio-stego mono round trip".toByteArray(Charsets.UTF_8)
    ...
    // sites before task #18 wired it, so this is the test that would catch a wrong-channel-count
    ...
    fun audioStegoStereoCarrierPayloadSurvivesTheFireflyMediaStoreRoundTrip() {
        ...
        val payload = "firefly gate-17 audio-stego STEREO round trip - the ravens have landed."
            .toByteArray(Charsets.UTF_8)
    ...
    fun acousticCarrierPayloadSurvivesTheFireflyMediaStoreRoundTrip() {
        ...
        val payload = "firefly gate-17 acoustic round trip".toByteArray(Charsets.US_ASCII)
