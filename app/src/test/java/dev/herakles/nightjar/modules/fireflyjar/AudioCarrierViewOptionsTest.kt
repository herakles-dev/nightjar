...
/**
 * v5 addition. Plain JUnit4, no Robolectric — [audioCarrierViewOptions]
 * is pure `String?`/`Int` -> `List<AudioCarrierView>`, no Compose or Android type involved.
 *
 * Restated as a test: "difference" only ever appears for a
 ...
 * function doesn't get to assume away — offers only the two baseline views with no new
 * option.
 */
