...
/**
 * "No riddle text is shown from anywhere but a successful decode" -- static source check,
 ...
 * this check actually governs -- NOT the glosses/hints/skip/restart copy, which are ordinary UI
            ...
            for ((name, value) in riddleValues) {
                assertFalse(
                    "${sourceFile.name} must not hardcode string resource \"$name\"'s text " +
                        "(\"$value\") -- riddle text may only reach the app through a " +
            ...
            for (line in sourceFile.readLines()) {
                ...
                assertFalse(
                    "${sourceFile.name} declares a public function returning String: \"$trimmed\" " +
                        "-- trail/'s public API must never return riddle text",
