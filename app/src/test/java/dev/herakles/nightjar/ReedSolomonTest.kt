 ...
 * framing on (architecture.md §4). This is deliberately independent of [AcousticCarrierTest]'s
 ...
 * two concrete shapes architecture.md §4 specifies (RS(14,6) header, RS(48,32) payload blocks,
 * including a shortened block) plus randomized shapes.
 *
 * The `decode` tests below (task #6) are the dedicated, isolated check for
...
class ReedSolomonTest {
    ...
    fun `RS(48,32) shortened final block still has roots at alpha^0 through alpha^15`() {
        // e.g. a 14-byte-of-payload+trailer tail block, per architecture.md §4's shortening rule.
    ...
    // ---------------------------------------------------------------------
    // decode() (task #6): zero-error fast path, single/multi byte-error correction,
