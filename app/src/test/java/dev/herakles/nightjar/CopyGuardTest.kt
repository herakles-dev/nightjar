...
/**
 * spec.md gate-39: no user-facing string may say "watching jar" or "framed jar" — both are
...
class CopyGuardTest {
    ...
    fun `no Kotlin string literal says watching jar or framed jar`() {
        ...
        assertTrue(
            "found banned copy in Kotlin string literals (gate-39 -- \"the meadow\"/\"the art jar\" " +
    ...
    fun `no Android string resource says watching jar or framed jar`() {
        ...
        assertTrue(
            "found banned copy in a values*/*.xml resource (gate-39):\n${violations.joinToString("\n")}",
            violations.isEmpty(),
        )
    }
