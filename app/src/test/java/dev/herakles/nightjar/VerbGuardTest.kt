...
/**
 * spec.md gate-40: the v6 verb sweep. design/riddle-trail.md § "Verb rule (owner direction,
 ...
 *
 * Two checks, in the same style [CopyGuardTest] already established for gate-39 (scan every
...
class VerbGuardTest {
    ...
    fun `no user-facing string says catch a firefly`() {
        ...
        assertTrue(
            "found the old embed verb \"catch a firefly\" (design/riddle-trail.md § Verb rule -- " +
    ...
    fun `creation-success copy says created, never caught`() {
        ...
        assertTrue(
            "creation-success copy must read \"you created one\" (design/riddle-trail.md § Verb " +
                "rule); \"you caught one\" on its own, with no trailing byte count, is still " +
