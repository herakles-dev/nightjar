...
/**
 * Plain JVM verification of [spectrogram] -- no Android/Bitmap dependency,
...
class SpectrogramTest {
    ...
    fun `a pure sine tone peaks at the FFT bin its frequency maps to`() {
        // 1875 Hz -- the AUDIBLE protocol's own F0 (Tone plan), bin-exact by
    ...
    // ---------------------------------------------------------------------
    // Grounds the MFSK honesty claim: AudioStegoCarrier.kt's MFSK_BASE_BIN (420,
    ...
    // ---------------------------------------------------------------------
    // AudioStegoCarrier's own class KDoc says mono-summing a
    ...
    // ---------------------------------------------------------------------
    // The test above only exercises full-scale NOISE
    ...
    // carried over from a scratch-prototype measurement (0.180 nats = 1.5 *
