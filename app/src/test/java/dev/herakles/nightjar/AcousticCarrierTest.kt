...
/**
 * Structural verification of [AcousticCarrier.encode], plus the [AcousticCarrier.decode]
 * round-trip and failure-path tests. The structural tests independently recompute the
...
class AcousticCarrierTest {
    ...
    // ---------------------------------------------------------------------
    // Round trip: encode(payload) -> decode(carrier) == payload
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // Round trip with leading capture offset: a real AudioRecord capture
    ...
    fun `round trip recovers payload when the transmission starts about 12s into a realistic 20s capture window`() {
        ...
        // The coarse search is one FFT per candidate frame over up to ~20s of
    ...
    // ---------------------------------------------------------------------
    // Round trip with a NON-frame-aligned leading offset. The offset-A/B/combined tests
    // above all happen to use leading lengths that are themselves exact multiples of
    ...
    // the header-declared-length END-marker location (decode()'s KDoc step 3) is for.
    // ---------------------------------------------------------------------
    ...
    // ---------------------------------------------------------------------
    // decode() failure paths: truncated / corrupted captures
    // ---------------------------------------------------------------------
    ...
    // Corruption is injected by silencing the single analysis frame decode() reads
    ...
    fun `decode fails closed when payload corruption exceeds the RS(48,32) correction ceiling`() {
        ...
        // unshortened RS(48,32) block, so every corrupted byte below lands
