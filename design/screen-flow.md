...
| **squeezed** | an exact-image firefly, but the file's own bytes no longer match raw-LSB's exact frame — consistent with the sending app having recompressed/resized it | `{app} squeezed this on the way, so the exact bits didn't survive. ask them to send it as a file, or use a sturdy firefly next time.` — when the sending app can't be identified from the intent, falls back to `whatever app carried this squeezed it on the way, so the exact bits didn't survive. ask them to send it as a file, or use a sturdy firefly next time.` |
